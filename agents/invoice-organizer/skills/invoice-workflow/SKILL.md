...
4. **解析能力**：不需要额外自检。PDF / 图片都由 `Read` 直接处理；`.ofd` 需要先用 `Bash` 的 `unzip` 展开再 `Read` 里面的 XML（`Read` 对 `.ofd` 本身不做任何解析），具体做法见 `invoice-extract`。都不需要 Python、不需要安装任何额外依赖（`unzip` 系统自带）。只有生成 `.xlsx` 台账才可能需要额外依赖，见 `invoice-ledger`。
