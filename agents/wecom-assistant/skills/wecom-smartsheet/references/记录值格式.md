...
| 参数 | 类型 | 必填 | 说明 |
|------|------|:----:|------|
| `media_id` | string | 是 | 媒体文件 ID，用户的消息中主动提供，或通过 `wecom-media` 的 `media upload` 获取 |
...
| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `userId` | string | userid。读取时必返；写入时，若按 `userName` 写入失败，则必须通过 `wecom-contact` 获取 `userid` 并传入此字段 |
