...
`references/建表模板.md` 使用 `FIELD_TYPE_*` 常量描述字段类型。调用 `wecom-cli smartsheet sheets add`、`fields add` 或 `fields update` 时，以本节上方“字段类型枚举”表为唯一依据，将模板常量转换为表中的 `参数值`，不能把 `FIELD_TYPE_*` 原样传给接口。
...
`FormulaItem` 和 `Formatter` 的定义和用法请参考 `references/公式字段.md`
