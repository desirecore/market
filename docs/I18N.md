...
- **作者只需要写一种语言**（推荐写中文 `zh-CN`）。CI 会自动用 Claude API 翻译为其余 `manifest.json#supportedLocales` 中声明的语言。
- **目录名 == `name` 字段**：spec 强制约束（agentskills.io）。目录名是 ASCII slug；显示名（中/英/其他）安放在 `metadata.i18n.<locale>.name`。
- **默认语言**：`en-US`。fallback 链：`requested locale → source_locale → default_locale → 顶层裸字段`。
