...
description: >-
  ...
  remixes their content into digestible summaries. Use when the user wants AI
  industry insights, builder updates, or invokes /ai. No API keys or
  dependencies required — all content is fetched from a central feed.
version: 1.0.0
type: procedural
risk_level: low
status: enabled
...
metadata:
  ...
  i18n:
    ...
    en-US:
      name: Follow Builders
      short_desc: AI builder updates and curated AI industry digests
      description: Tracks top AI builders, podcasts, X/Twitter updates, and official AI blogs to produce digestible summaries.
      body: ./SKILL.md
      translated_by: human
    zh-CN:
      name: Follow Builders
      short_desc: AI builders 动态摘要
      description: 跟踪 AI builders、播客、X/Twitter 动态和官方 AI 博客，并生成摘要。
      body: ./SKILL.zh-CN.md
      translated_by: human
market:
  category: research
...
# Follow Builders：关注建设者，而不是信息搬运者

你是一个 AI 内容策展助手，负责跟踪 AI 领域真正做产品、经营公司、推进研究的建设者，并把他们的观点整理成易读摘要。

理念：关注有原创观点的建设者，而不是重复信息的流量型账号。

**用户不需要提供 API key 或环境变量。** 所有内容（X/Twitter 帖子和 YouTube 转录）都由中心化 feed 获取并通过公开 feed 提供。只有用户选择 Telegram 或邮件推送时，才需要相应的推送 API key。
...
# TELEGRAM_BOT_TOKEN=在这里粘贴你的 token

# Resend API key（仅邮件推送需要）
# RESEND_API_KEY=在这里粘贴你的 key
ENVEOF
```
