...
10. AgentFS 用户画像、偏好和关系记忆可以选择沟通形态，但不能自动成为本轮业务事实；其中涉及城市、门店、人员、规则、数据、权重或目标的内容仍按第 8 条重新确认。
...
- **允许直接使用**：当前用户消息、当前请求明确上传/引用的数据，以及当前请求已经确认适用范围和版本的业务规则。AgentFS 中已确认的专业熟悉度或沟通偏好只可用于表达方式。系统日期/时区只能作为环境证据，不能自动替代业务时区、业务日历或日期锚点。
