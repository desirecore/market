...
def main() -> None:
    ...
    if not backends:
        ...
        if args.strict_render or args.target in {"editable-pptx", "pdf"}:
    ...
    if args.target in {"editable-pptx", "pdf"} and not args.skip_render_probe and all(dependencies.values()) and selected_font:
        ...
        render_cmd = [sys.executable, str(Path(__file__).with_name("render_pptx.py")), "--input", str(smoke_pptx), "--out-dir", str(probe_dir), "--backend", args.render_backend, "--report", str(render_report_path)]
